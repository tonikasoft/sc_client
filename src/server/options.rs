...
impl Options {
    ...
    fn set_config_defaults(config: &mut Config) -> Result<Config, ConfigError> {
        ...
        Ok(config
            ...
            .set_default(
                "input_streams_enable_string",
                defaults.input_streams_enable_string,
            )?
            .set_default("load_synth_defs", defaults.load_synth_defs)?
            .set_default(
                "max_interconnect_buffers",
                defaults.max_interconnect_buffers as i64,
            )?
            ...
            .set_default(
                ...
                defaults.num_audio_bus_channels as i64,
            )?
            .set_default("num_buffers", defaults.num_buffers as i64)?
            .set_default(
                "num_control_bus_channels",
                defaults.num_control_bus_channels as i64,
            )?
            .set_default(
                "num_input_bus_channels",
                defaults.num_input_bus_channels as i64,
            )?
            .set_default("num_of_threads", defaults.num_of_threads as i64)?
            .set_default(
                "num_output_bus_channels",
                defaults.num_output_bus_channels as i64,
            )?
            .set_default(
                "output_streams_enable_string",
                defaults.output_streams_enable_string,
            )?
            .set_default("path", defaults.path)?
            .set_default(
                "preferred_hardware_buffer_size",
                defaults.preferred_hardware_buffer_size as i64,
            )?
            .set_default(
                "preferred_sample_rate",
                defaults.preferred_sample_rate as i64,
            )?
            .set_default("publish_to_rendezvous", defaults.publish_to_rendezvous)?
            .set_default(
                "random_number_generators",
                defaults.random_number_generators as i64,
            )?
            .set_default(
                "real_time_memory_size",
                defaults.real_time_memory_size as i64,
            )?
    ...
    fn on_error_reading_config(e: ConfigError) -> Config {
        ...
        let defaults = Options::default();
        Config::try_from::<Options>(&defaults).expect("Cannot init config from default Options")
    }

    pub fn to_args(&self) -> Vec<String> {
        let mut result = vec![
            Options::get_arg_with_value_or_empty_vec("-H", self.device_name.clone()),
            Options::get_arg_with_value_or_empty_vec(
                "-I",
                self.input_streams_enable_string.clone(),
            ),
            Options::get_arg_with_value_or_empty_vec(
                "-O",
                self.output_streams_enable_string.clone(),
            ),
            ...
            vec![
                String::from("-D"),
                (self.load_synth_defs as i32).to_string(),
            ],
            vec![
                String::from("-R"),
                (self.publish_to_rendezvous as i32).to_string(),
            ],
            vec![String::from("-S"), self.preferred_sample_rate.to_string()],
            vec![String::from("-V"), self.verbosity.to_string()],
            vec![
                String::from("-Z"),
                self.preferred_hardware_buffer_size.to_string(),
            ],
    ...
    fn update_configs_default_sr_by_server_type(
        config: &mut Config,
        server_type: &str,
    ) -> Result<(), ConfigError> {
